...
import (
	...
	mcptools "nexus-mcp/pkg/mcptools"

	"nexus-mcp/pkg/mcpvirtual/engine"
	"nexus-mcp/pkg/mcpvirtual/pipeline"
)
...
func loadVirtualTools() map[string]pipeline.VirtualToolEntry {
	cfgPath := mcputils.VirtualConfigPath("nexus-mcp")
	aggEngine, err := engine.New(cfgPath, &virtualRegistry{}, nil)
	if err != nil {
...
func ListTools() {
	cfg, _ := mcputils.LoadConfig("nexus-mcp")
	...
	if len(nativeTools) == 0 {
		fmt.Fprintf(os.Stderr, "Warning: no native tools are enabled. To expose native tools, set nativeTools.expose.register_all_tools_by_default: true or add tool names to tools.expose.includes in ~/.nexus-mcp/config.yaml\n")
	}
