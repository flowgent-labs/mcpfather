...
// TestServer_401WithoutToken verifies the MCP server returns 401 when no
// Authorization header is present.
func TestServer_401WithoutToken(t *testing.T) {
...
// TestServer_401MalformedHeader verifies the MCP server rejects non-Bearer
// Authorization headers (e.g. Basic auth) with 401.
func TestServer_401MalformedHeader(t *testing.T) {
	issuer, cleanupKC := ensureKeycloak(t)
	defer cleanupKC()
...
// TestServer_401HS256Forgery verifies algorithm confusion prevention:
// the server only trusts RSA keys from JWKS and must reject HMAC-signed tokens.
func TestServer_401HS256Forgery(t *testing.T) {
	issuer, cleanupKC := ensureKeycloak(t)
	defer cleanupKC()
...
// TestServer_WellKnownMetadata verifies the RFC 9728 protected resource
// metadata endpoint.
func TestServer_WellKnownMetadata(t *testing.T) {
	issuer, cleanupKC := ensureKeycloak(t)
	defer cleanupKC()
...
// TestServer_StdioNoAuth verifies that stdio transport emits a warning
// about frontend auth being ignored (rather than crashing).
func TestServer_StdioNoAuth(t *testing.T) {
	issuer, cleanupKC := ensureKeycloak(t)
	defer cleanupKC()
...
`, mock.server.URL, issuer)
	...
	if !strings.Contains(stderr, "Warning") || !strings.Contains(stderr, "server") {
...
// TestServer_DeviceCodeFlow_Keycloak tests the full RFC 8628 device_code
// flow against a real Keycloak container.
func TestServer_DeviceCodeFlow_Keycloak(t *testing.T) {
...
// TestServer_ClientCredentialsFlow_Keycloak tests the full client_credentials
// (machine-to-machine) flow against a real Keycloak container.
func TestServer_ClientCredentialsFlow_Keycloak(t *testing.T) {
	issuer, cleanup := ensureKeycloak(t)
	defer cleanup()
...
// TestServer_401ExpiredToken verifies 401 rejection for an expired JWT.
func TestServer_401ExpiredToken(t *testing.T) {
...
// TestServer_401WrongAudience verifies 401 rejection when audience doesn't match.
func TestServer_401WrongAudience(t *testing.T) {
	oidc := startMockOIDCServer(t)
	defer oidc.Close()
...
// TestServer_401WrongIssuer verifies 401 rejection when issuer doesn't match.
func TestServer_401WrongIssuer(t *testing.T) {
	oidc := startMockOIDCServer(t)
	defer oidc.Close()
...
// TestServer_ClientTokenForwarding verifies that validated JWT claims (sub, email)
// are forwarded upstream as X-Mcp-Client-Token-Sub / X-Mcp-Client-Token-Email headers.
func TestServer_ClientTokenForwarding(t *testing.T) {
	issuer, cleanup := ensureKeycloak(t)
	defer cleanup()
...
// TestServer_ClientTokenForwardingDisabled verifies that when
// enable_client_token_claim_forward is false, no claim headers are sent.
func TestServer_ClientTokenForwardingDisabled(t *testing.T) {
	issuer, cleanup := ensureKeycloak(t)
	defer cleanup()
