...
IT_BATCH_TOTAL := 12
TEST_PROXY_PRESENT := $(strip $(HTTPS_PROXY)$(https_proxy)$(HTTP_PROXY)$(http_proxy)$(MCPFATHER_TEST_PROXY))
ifeq ($(TEST_PROXY_PRESENT),)
TEST_GO_NETWORK_ENV := GOPROXY=$(GOPROXY)
else
TEST_GO_NETWORK_ENV :=
endif
...
test-ut:
	$(TEST_GO_NETWORK_ENV) go test -v -count=1 -timeout 300s ./pkg/... ./cmd/...

test-it:
	IT_BATCH='01/$(IT_BATCH_TOTAL) core-generator-auth' $(TEST_GO_NETWORK_ENV) GOMAXPROCS=$(IT_GOMAXPROCS) GOFLAGS=$(IT_GOFLAGS) go test $(IT_TEST_FLAGS) ./it/ -run '$(IT_CORE_GEN_AUTH_RE)'
	IT_BATCH='02/$(IT_BATCH_TOTAL) core-logging-download' $(TEST_GO_NETWORK_ENV) GOMAXPROCS=$(IT_GOMAXPROCS) GOFLAGS=$(IT_GOFLAGS) go test $(IT_TEST_FLAGS) ./it/ -run '$(IT_CORE_LOG_DOWNLOAD_RE)'
	...
	IT_BATCH='10/$(IT_BATCH_TOTAL) oidc-upstream-client' $(TEST_GO_NETWORK_ENV) GOMAXPROCS=$(IT_GOMAXPROCS) GOFLAGS=$(IT_GOFLAGS) go test $(IT_TEST_FLAGS) ./it/ -run '$(IT_OIDC_UPSTREAM_RE)'
	IT_BATCH='11/$(IT_BATCH_TOTAL) deploy-full-e2e' $(TEST_GO_NETWORK_ENV) GOMAXPROCS=$(IT_GOMAXPROCS) GOFLAGS=$(IT_GOFLAGS) go test $(IT_TEST_FLAGS) ./it/ -run '$(IT_DEPLOY_FULL_RE)'
	IT_BATCH='12/$(IT_BATCH_TOTAL) deploy-helm-ingress' $(TEST_GO_NETWORK_ENV) GOMAXPROCS=$(IT_GOMAXPROCS) GOFLAGS=$(IT_GOFLAGS) go test $(IT_TEST_FLAGS) ./it/ -run '$(IT_DEPLOY_MISC_RE)'
