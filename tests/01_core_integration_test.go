...
// genProjectWithSpec runs mcpgen with a custom spec file (relative to tests/).
func genProjectWithSpec(t *testing.T, specFile, includes, excludes string) string {
	t.Helper()
	bin := mcpgenBin(t)
	dir := t.TempDir()
	args := []string{"-i", filepath.Join(repoRoot(t), "tests", specFile), "-o", dir}
	if includes != "" {
...
func TestGenerator_Includes_NonExistentOperationId_Errors(t *testing.T) {
	bin := mcpgenBin(t)
	dir := t.TempDir()
	spec := filepath.Join(repoRoot(t), "tests", specFixture)
